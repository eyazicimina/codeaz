...
df = pd.read_excel("BankChurners_original.xlsx")
...
#skew
"""
for c in ['Total_Trans_Amt']:
	data1 = df[0:5000][c]
	data2 = df[5000:][c]
	# Is there any difference between new and old customer's Credit Limit
...
#: Get the skewness, kurtosis
skewness_values = df.skew(numeric_only=True).to_dict()
kurtosis_values = df.kurt(numeric_only=True).to_dict()

# for each value (column name), find the df[value].nunique()
statistics['distinct'] = statistics['columnname'].apply( lambda value: df[value].nunique() )
statistics['datatype'] = statistics['columnname'].apply( lambda value: df.dtypes[value] )
statistics['cardinality'] = statistics['distinct'] / statistics['count']
# ALTERNATIVE statistics['skewness'] = statistics['columnname'].apply( lambda value: skewness_values[value] )
statistics['skewness'] = statistics['columnname'].map( skewness_values )
statistics['kurtosis'] = statistics['columnname'].map( kurtosis_values )

statistics['correlation'] = statistics['columnname'].apply( lambda value: df['Churn'].corr( df[value] ))


statistics.to_csv("statistics2.csv")


print( df['Churn'].corr( df['Total_Trans_Amt'] ) )
# CORREL = excel, df[first].corr(df[second])
